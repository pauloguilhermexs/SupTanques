...
void SupClienteQt::on_actionLogout_triggered()
{
  ...
  /* ACRESCENTAR */ SupCliente::desconectar();
}
...
void SupClienteQt::on_buttonV1_clicked(bool open)
{
  ...
  /* ACRESCENTAR */ SupCliente::setV1Open(open);
}
...
void SupClienteQt::on_buttonV2_clicked(bool open)
{
  ...
  // Jah exibe msg de erro em caso de insucesso.
  /* ACRESCENTAR */SupCliente::setV2Open(open);
}
...
void SupClienteQt::on_sliderPump_valueChanged(int value)
{
  ...
  // Jah exibe msg de erro em caso de insucesso.
  /* ACRESCENTAR */ SupCliente::setPumpInput(static_cast<uint16_t>(value));
...
void SupClienteQt::on_spinRefresh_valueChanged(int arg1)
{
  // Altera o intervalo entre solicitacoes de dados
    /* ACRESCENTAR */SupCliente::setTimeRefresh(arg1);
}
...
void SupClienteQt::slotConectar(QString IP, QString Login, QString Senha)
{
  ...
  /* ACRESCENTAR */ SupCliente::conectar(IP.toStdString(), Login.toStdString(), Senha.toStdString());
}
...
void SupClienteQt::slotExibirInterface()
{
  ...
  bool estahConectado = SupCliente::isConnected(); /* MODIFICAR *//*false*/;
  // Booleano que indica se cliente eh administrador ou nao.
  // Chama funcao que retorna essa informacao.
  bool ehAdministrador =SupCliente::isAdmin(); /* MODIFICAR *//*false*/;
  ...
  if (estahConectado)
  {
    ...
    if (estavaConectado <=0)
    {
      ...
      QString msg = QString(" CONNECTED: ") + QString::fromStdString(SupCliente::meuUsuario/* MODIFICAR "NOME_USUARIO" */) +
          " (" + (ehAdministrador ? "admin" : "viewer") + ")";
      statusMsg->setText(msg);
    }

    // Exibe nos visualizadores o ultimo estado lido da planta.
    showValves(SupCliente::lastState().V1/* MODIFICAR 0 */, SupCliente::lastState().V2/* MODIFICAR 0 */);
    showPump(SupCliente::lastState().PumpInput/* MODIFICAR 0 */);
    showH(SupCliente::lastState().H1/* MODIFICAR 0 */,SupCliente::lastState().H2 /* MODIFICAR 0 */, SupCliente::lastState().ovfl /* MODIFICAR 0*/);
    showFlow(SupCliente::lastState().PumpFlow/* MODIFICAR 0 */);
